...
### Текущий статус: Фаза 4 — В процессе 🔄
...
| **Фаза 4** | 3 недели | 🔄 В процессе | 20% | Отчёты и расширенные E2E |
| **Фаза 5** | 1 неделя | ⏳ Ожидается | 0% | Финализация и деплой |

📖 **Детальная дорожная карта:** [PROJECT_PHASES_RU.md](./docs/PROJECT_PHASES_RU.md)
...
**Статус:**
- 🔄 Фаза 4 запущена, задачи заведены в GitHub
- 🔄 Колонка In Progress: #92 (Admin UI: тарифы), #119 (конфигурируемый rate-limit/brute-force), #120 (правки API-контрактной документации), #121 (удаление устаревших DB migration доков), #122 (консолидация JWT/auth документации), #123 (точечные исправления документации)
- 🔄 Добавлен архитектурный трек: #125–#133 (усиление JWT, фрагментация схем БД, edge/resilience/concurrency hardening)

### 🧭 Принятые архитектурные изменения (запланированы в Phase 4)

- **JWT-модель:** переход с HS512 (общий secret) на **RS256 + JWKS** с ротацией ключей (#125, #127)
- **Межсервисная аутентификация:** единый стандарт user-token relay и service-token для фоновых потоков (#126)
- **Изоляция данных:** переход от общей схемы к **schema-per-service** + least-privilege DB users (#128, #129, #130)
- **Надёжность и edge-cases:** формализация auth edge-сценариев, timeout/retry/circuit-breaker и race-condition обработки (#131, #132, #133)
...
- **Текущая модель**: единая shared schema для всех сервисов
- **План (Phase 4)**: изоляция schema-per-service в том же PostgreSQL-инстансе (issues #128–#130)
...
Текущая реализация использует HS512 с общим секретом. В Phase 4 запланирован переход на RS256/JWKS и строгие правила валидации межсервисных токенов (issues #125–#127).
