...
### Текущий backlog из ToDo колонки репозитория
Следующие открытые задачи Phase 4 сейчас отслеживаются в GitHub backlog и должны рассматриваться как часть плана фазы наряду с дорожной картой выше:

| ID | Задача | Статус | Issue |
|----|--------|--------|-------|
| 4.5 | **Разорвать зависимость от общей БД** | ⏳ | #98 |
| 4.6 | **Защитить межсервисное взаимодействие** | ⏳ | #97 |
| 4.7 | **Усилить управление секретами** | ⏳ | #96 |
| 4.8 | **Заменить hardcoded URL сервисов** | ⏳ | #99 |
| 4.9 | **Определить bounded contexts и API contracts** | ⏳ | #100 |
| 4.10 | **Ввести асинхронные события** | ⏳ | #101 |
| 4.11 | **Стандартизировать профили окружения** | ⏳ | #102 |
| 4.12 | **Улучшить observability и readiness** | ⏳ | #103 |
| 4.13 | **Снизить дублирование в build/deployment** | ⏳ | #104 |
| 4.14 | **Удалить чувствительные файлы из git-истории (login.json, ssh key.txt)** | ⏳ | #107 |
| 4.15 | **Убрать fallback-значение JWT_SECRET — fail-fast при старте** | ⏳ | #108 |
| 4.16 | **Закрыть тестовые эндпоинты billing-service через @Profile("test")** | ⏳ | #109 |
| 4.17 | **Вынести AuditLogger в parking-common (устранить 3 копии)** | ⏳ | #110 |
| 4.18 | **Устранить дублирование JwtTokenProvider в client-service и reporting-service** | ⏳ | #111 |
| 4.19 | **Унифицировать версию jjwt до 0.12.6 во всех сервисах** | ⏳ | #112 |
| 4.20 | **Добавить GlobalExceptionHandler в gate-control-service и management-service** | ⏳ | #113 |
| 4.21 | **Исправить мёртвый код и логические ошибки в API-слое frontend** | ⏳ | #114 |
| 4.22 | **Добавить @Valid к параметрам request body в billing-service контроллере** | ⏳ | #115 |
| 4.23 | **Архивировать директорию docs/sessions** | ✅ | #116 |
| 4.24 | **Добавить скилл docs-sync и спланировать рефакторинг Phase 4** | ⏳ | #117 |
| 4.25 | **Требовать от docs-sync обновлять RU-версии при изменении EN-доков** | ✅ | #124 |
| 4.26 | **Определить целевую модель доверия JWT (RS256/JWKS) и план миграции** | ⏳ | #125 |
| 4.27 | **Стандартизировать проброс токенов в межсервисных вызовах** | ⏳ | #126 |
| 4.28 | **Ввести строгую валидацию JWT и негативные тесты** | ⏳ | #127 |
| 4.29 | **Спроектировать модель schema-per-service в общем PostgreSQL** | ⏳ | #128 |
| 4.30 | **Ввести отдельных DB-пользователей сервисов и права least-privilege** | ⏳ | #129 |
| 4.31 | **Выполнить поэтапный перенос данных из общих таблиц в bounded schemas** | ⏳ | #130 |
| 4.32 | **Явно обработать edge-сценарии auth/security** | ⏳ | #131 |
| 4.33 | **Добавить стратегию timeout/retry/circuit-breaker для межсервисных вызовов** | ⏳ | #132 |
| 4.34 | **Покрыть race conditions в потоках gate/billing/parking capacity** | ⏳ | #133 |
...
**Текущий фокус:** завершение Admin UI, Reporting parity, hardening архитектуры, стандартизация deployment и observability  
